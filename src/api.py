...
from src.processing import (load_data,transform__df,process_fact_table,process_dim_table_df,get_top_sellers,get_loyal_customers)
from sqlalchemy.orm import Session
from src.database import (get_db, Sellers,Customers,Orders,Order_Items,Products,Product_Category,FactTable,Order_Payments,Top_Sellers,Loyal_Customers)
    ...
    try:
        with get_db() as db:
            try:
                ...
                print("loyal_customers_df: ", loyal_customers_df)
                ...
                loyal_customers_ids = [record['id'] for record in list_dicts_df]
                existing_loyal_customers = db.query(Loyal_Customers).filter(Loyal_Customers.id.in_(loyal_customers_ids)).all()
                logger.info(f"Found {len(existing_loyal_customers)} existing loyal_customers")
                # Create a set of existing loyal_customers IDs for faster lookup
                existing_loyal_customers_ids = {top.id for top in existing_loyal_customers}
                ...
                for loyal_customers in new_loyal_customers:
                    loyal_customers_dict = {}
                    loyal_customers_dict['id'] = loyal_customers.id
                    loyal_customers_dict['customer_unique_id'] = loyal_customers.customer_unique_id
                    loyal_customers_dict['total_orders'] = loyal_customers.no_of_orders

                    loyal_customers_list.append(loyal_customers_dict)
...
def api_get_loyal_customers():
    with get_db() as db:
        try:
            loyal_customers = db.query(Loyal_Customers).all()

            loyal_customers_list = []

            if  loyal_customers:
                for loyal_customers in loyal_customers:
                    loyal_customers_list.append({
                        'id': loyal_customers.id,
                        'customer_unique_id': loyal_customers.customer_unique_id,
                        'total_orders': loyal_customers.no_of_orders
                    })
