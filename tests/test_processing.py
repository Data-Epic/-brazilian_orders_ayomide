...
class TestProcessDimTableDF(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_db_table = [
            MockDBRow(
                id=1, name="Peter", job="Data Engineer"
            ),
            MockDBRow(
                id=2, name="John", job="Data Scientist"
            ),
            MockDBRow(
                id=3, name="Jane", job="Software Engineer"
            )
        ]
...
class TestGetTopSellingProductCategory(unittest.TestCase):
    ...
    def test_with_correct_data(self):
        ...
        # Check if the dataframe has the correct columns
        self.assertListEqual(result.columns, ["id","product_category_name_english", "Total_sales"])
        ...
        for col in result.columns:
            self.assertEqual(set(result[col].to_list()), set(expected_df[col].to_list()))
            print("result", result[col].to_list())
                             
    def test_empty_dataframe(self):
        "Test the get_top_selling_product_category function parameter with an empty DataFrame"
        empty_df = pl.DataFrame()
        result_df = get_top_selling_product_category(empty_df)
        self.assertEqual(result_df, "Dataframe is empty, Provide the valid Fact table")

    def test_with_string(self):
        "Test the get_top_selling_product_category function parameter with a string type"
    ...
    def test_with_none(self):
        ...
        "Test the get_top_selling_product_category function parameter with a None type"
...
class TestGetOrdersStatusCount(unittest.TestCase):
    ...
    def test_empty_dataframe(self):
        "Test the get_orders_status_count function parameter with an empty DataFrame"
        empty_df = pl.DataFrame()
        result_df = get_orders_status_count(empty_df)
        self.assertEqual(result_df, "Dataframe is empty, Provide the valid Fact table")

    def test_with_string(self):
        "Test the get_orders_status_count function parameter with a string type"
...
class TestGetAverageDeliveryDuration(unittest.TestCase):
    ...
    def test_with_correct_data(self):
        ...
        # Check if the dataframe has the correct columns
        self.assertListEqual(result.columns, ["id","product_category_name_english", "Average_delivery_duration_days"])
        ...
        for col in result.columns:
            print("result", result[col].to_list())
            print("expected", expected_df[col].to_list())
            self.assertEqual(set(result[col].to_list()), set(expected_df[col].to_list()))
...
class TestGetLoyalCustomers(unittest.TestCase):
    ...
    def test_with_correct_data(self):
        ...
        # Check if the dataframe has the correct columns
        self.assertListEqual(result.columns, ["id","customer_unique_id", "no_of_orders"])
